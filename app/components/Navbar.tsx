...
const Navbar: React.FC = () => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 ${isOpen
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20 md:h-24">
          ...
          <Link href="/" onClick={() => setIsOpen(false)} className="group flex flex-col items-start z-[110]">
            <span className={`text-2xl md:text-3xl font-black tracking-tighter leading-none transition-colors duration-300 uppercase ${isOpen ? 'text-white' : (scrolled ? 'text-slate-900' : 'text-slate-900 md:text-slate-900')
              }`}>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.path}
                  href={link.path}
                  className={`relative px-5 py-2 text-sm font-bold tracking-wide transition-colors duration-300 hover:text-indigo-600 ${isActive ? 'text-indigo-600' : 'text-slate-600'
                    }`}
                >
                  {link.name}
                  {isActive && (
                  ...
                  {navLinks.map((link, idx) => (
                    ...
                    >
                      <Link
                        href={link.path}
                        onClick={() => setIsOpen(false)}
                        className={`flex items-center gap-3 text-lg font-bold tracking-tight transition-all ${pathname === link.path ? 'text-indigo-400' : 'text-white/90 hover:text-white'
                          }`}
                      >
