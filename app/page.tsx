...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 selection:bg-indigo-100 selection:text-indigo-900 overflow-hidden relative">
      {/* Promotional Banner for First Ride */}
      <motion.div 
        initial={{ y: -50, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ delay: 1.5, type: 'spring', stiffness: 200, damping: 20 }}
        className="absolute top-24 md:top-32 left-4 right-4 md:left-auto md:right-8 z-50 cursor-pointer hover:scale-105 transition-transform"
      >
        <Link href="/contact" className="block">
          <div className="bg-gradient-to-br from-amber-500 to-amber-600 rounded-2xl shadow-2xl shadow-amber-500/20 p-4 flex items-center gap-4 md:w-80 border border-amber-400 relative overflow-hidden">
            <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10 mix-blend-overlay" />
            <div className="bg-white/20 p-2.5 rounded-full shrink-0 relative z-10 backdrop-blur-sm border border-white/20">
              <Star className="w-5 h-5 text-white" fill="currentColor" />
            </div>
            <div className="relative z-10">
              <h4 className="text-white font-black text-xs md:text-sm uppercase tracking-widest drop-shadow-sm mb-0.5">Welcome Offer</h4>
              <p className="text-amber-50 font-medium text-sm leading-tight drop-shadow-sm">
                Get <span className="font-black text-white text-base bg-white/20 px-1.5 py-0.5 rounded-md mx-0.5 border border-white/20 backdrop-blur-sm">₹200 OFF</span> <br/>on your first ride!
              </p>
            </div>
            <div className="ml-auto bg-white/20 p-1.5 rounded-full shrink-0 relative z-10">
